...
class BABYBOT_DISCORD(PlatformIntegrationMixin, commands.Bot):
    ...
    def should_persist_user_state(self, user_key: str) -> bool:
        ...
        if self.is_bot_identity(key):
            return True
        if key in self.AIoptInUsers:
            return True

        # Check if user has active economy/game state
        mem = self.userMemory.get(key)
        if isinstance(mem, dict):
            if abs(mem.get("BBY", 0.0)) > 0.001:
                return True
            for field in [
                "wins",
                "losses",
                ...
                if mem.get(field, 0.0) > 0:
                    return True
            inventory = mem.get("inventory")
            if inventory and isinstance(inventory, dict) and len(inventory) > 0:
                return True
            bbybook = mem.get("bbybook")
            if bbybook and isinstance(bbybook, list) and len(bbybook) > 0:
                return True
            if mem.get("maths_best_level", 1) > 1:
                return True
            if mem.get("loyalty", 1) > 1:
                return True
        return False
    ...
    def prune_non_opt_user_memory(
        self, reason: str = "", include_non_opt: bool = True
    ) -> int:
        removed_users = []
