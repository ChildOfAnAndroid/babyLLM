...
class VOCAB:
    def __init__(self, vocabSize = vocabSize, vocabPath = vocabLoad):
        ...
        with self.v_counsellor.infodump("__init__") as ʕっʘ‿ʘʔっ:
            ...
            if statPrints: print(f"vocab length: {len(self.tokenizer.get_vocab())}")

            if self.loadVocab():
                ʕっʘ‿ʘʔっ("loadVocab")
                if statPrints: print(f"loaded vocab: {self.vocabCache}")
                ...
                ʕっʘ‿ʘʔっ("buildVocab")
                if statPrints: print(f"building vocab from scratch (size: {vocabSize})...")
                self.buildVocabMap() # Use new method to build vocab mappings
                if statPrints: print(f"vocab length: {len(self.vocabList)}") # Vocab length from list
                ʕっʘ‿ʘʔっ("saveVocab")
                self.saveVocab()
    ...
    def loadVocab(self):
        with self.v_counsellor.infodump("loadVocab") as ʕっʘ‿ʘʔっ:
            try:
                ...
                if statPrints: print(f"first 20 tokens: {self.vocabList[:20]}")
