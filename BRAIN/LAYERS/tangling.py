...
class TANGLING(nn.Module):
    def __init__(self, _counsellor, _attention2_reference, _device=modelDevice):
        ...
        self.project_up = nn.Linear(embedDimension, numNeurons, device=self.device)      # 1024 → 10000
        self.project_down = nn.Linear(numNeurons, embedDimension, device=self.device)    # 10000 → 1024
        ...
        self.embed_tangle_gate = nn.Parameter(torch.tensor(gate_init, device=self.device))
        self.neuron_tangle_gate = nn.Parameter(torch.tensor(gate_init, device=self.device))  # Now learnable!
        self.memory_tangle_gate = nn.Parameter(torch.tensor(gate_init, device=self.device))
    ...
    def refine(self, _input, stage_name="unknown"):
        ...
        with self.counsellor.infodump("refine") as ʕっʘ‿ʘʔっ:
            ...
            if original_dim == numNeurons:
                ...
                x_10k = _input
                if debugPrints: ʕっʘ‿ʘʔっ(f"tangling {stage_name}: already 10k-dim")
            else:
                # Need to project up (embeddings or memory)
                x_10k = self.project_up(_input)
                if debugPrints: ʕっʘ‿ʘʔっ(f"tangling {stage_name}: projected {original_dim} → {numNeurons}")

            # Apply attention2 refinement (REUSING existing weights!)
            refined_10k = self.attention2(x_10k)
            if debugPrints: ʕっʘ‿ʘʔっ(f"tangling {stage_name}: attention2 refinement applied")
            ...
            if original_dim == numNeurons:
                ...
                if debugPrints: ʕっʘ‿ʘʔっ(f"tangling {stage_name}: projected {numNeurons} → {original_dim}")
            ...
            if stage_name == "embed":
                ...
                gate = torch.sigmoid(self.neuron_tangle_gate)  # Now gated like the others!
            ...
            try:
                with torch.no_grad():
                    ...
                    self.stats[f"TANGLE_{stage_name}_refinement_norm"] = gated_refinement.norm().item()
            except:
                pass
...
class MINI_INN_TANGLING(nn.Module):
    ...
    def __init__(self, _counsellor, _numTokensPerStep, _calligraphist=None, _device=modelDevice):
        super().__init__()
        self.counsellor       = _counsellor
        ...
        for stage in ("embed",):
            setattr(self, f"{stage}_norm",
                    nn.LayerNorm(embedDimension, device=_device))
            setattr(self, f"{stage}_proj_down",
                    nn.Linear(embedDimension, mn, device=_device, bias=False))
            setattr(self, f"{stage}_proj_up",
        ...
        for stage in ("neuron", "memory"):
            setattr(self, f"{stage}_norm",
                    nn.LayerNorm(numNeurons, device=_device))
            setattr(self, f"{stage}_proj_down",
        ...
        self.logWindowSizes = nn.Parameter(
            torch.log(torch.tensor(allWindowSizes_new, dtype=torch.float32, device=_device))
        )
        ...
        self.window_softmax_temperature = nn.Parameter(torch.tensor(-0.436, device=_device))

        # ── History buffers for stats ──
        self._history_attrs = [
            "embed_gate_hist",  "embed_norm_hist",
            "neuron_gate_hist", "neuron_norm_hist",
            "memory_gate_hist", "memory_norm_hist",
        ]
        ...
        if debugPrints:
            print(f"[MINI_INN_TANGLING] init: mn={mn}, mn_large={mn_large}, "
    ...
    def _build_window_tensor(self):
        ...
        wt = ((1 - fractionality) * intW + fractionality * floatW).unsqueeze(1)  # (W, 1)
    ...
    def _causal_window_means(self, x: torch.Tensor, windowTensor: torch.Tensor) -> torch.Tensor:
        """
        ...
        with self.counsellor.infodump("_causal_window_means") as ʕっʘ‿ʘʔっ:
            ...
            if debugPrints: ʕっʘ‿ʘʔっ(f"seq_len={seq_len}, dim={dim}, maxLen={maxLen}")
            ...
            if debugPrints: ʕっʘ‿ʘʔっ("gather start indices")
            ...
            if debugPrints: ʕっʘ‿ʘʔっ("gather cumsum end/start and compute means")
            ...
            means = (sum_end.unsqueeze(0) - sum_start) / counts.unsqueeze(-1)  # (W, actual_len, dim)
    ...
    def refine(self, _input, stage_name="unknown"):
        ...
        """
        with self.counsellor.infodump("refine") as ʕっʘ‿ʘʔっ:
            if debugPrints: ʕっʘ‿ʘʔっ(f"stage={stage_name}, input shape={_input.shape}")
            ...
            if stage_name in ("neuron", "memory"):
                ...
                if debugPrints: ʕっʘ‿ʘʔっ(f"MLP path: LN → {numNeurons}→{self.MINI_NEURONS_LARGE} → GELU → {numNeurons} → out_norm → gate")
                ...
                if debugPrints: ʕっʘ‿ʘʔっ(f"gate={gate.item():.4f}, gated norm={gated.norm().item():.4f}")
            ...
            if debugPrints: ʕっʘ‿ʘʔっ("layernorm")
            x = norm_layer(_input)

            # 2. Project to mini-neuron space and activate
            if debugPrints: ʕっʘ‿ʘʔっ(f"proj_down {embedDimension}→{self.MINI_NEURONS} + GELU")
            x = F.gelu(proj_down(x))                  # (seq_len, MINI_NEURONS)

            # 3. Build window tensor and compute causal per-position means
            if debugPrints: ʕっʘ‿ʘʔっ("build_window_tensor")
            windowTensor = self._build_window_tensor()                          # (W, 1)

            if debugPrints: ʕっʘ‿ʘʔっ("causal_window_means")
            window_means = self._causal_window_means(x, windowTensor)           # (W, seq_len, mn)

            # 4. Cerebellum: learned softmax blend over windows
            if debugPrints: ʕっʘ‿ʘʔっ("cerebellum softmax blend")
            ...
            if debugPrints: ʕっʘ‿ʘʔっ("internal residual (blended + x)")
            blended = blended + x                      # (seq_len, MINI_NEURONS)

            # 5. Project back to embedDimension + output LayerNorm
            if debugPrints: ʕっʘ‿ʘʔっ(f"proj_up {self.MINI_NEURONS}→{embedDimension} + out_norm")
            ...
            if debugPrints: ʕっʘ‿ʘʔっ(f"gate={gate.item():.4f}, gated shape={gated.shape}")
    ...
    def getTanglingStats(self):
        with self.counsellor.infodump("getTanglingStats") as ʕっʘ‿ʘʔっ:
            ...
            if debugPrints: ʕっʘ‿ʘʔっ("collecting cerebellum per-stage stats")
            try:
                with torch.no_grad():
                ...
                with torch.no_grad():
                    ...
                    for stage in ("embed",):
                        ...
                        if soft is None or raw is None:
                            ...
                            continue
                        if debugPrints: ʕっʘ‿ʘʔっ(f"♥{stage} cerebellumString")
                    ...
                    for stage in ("neuron", "memory"):
                        ...
                        if gate_val is not None:
                            parts.append(f"MINI_TANGLE_{stage}   gate={gate_val:.4f}   norm={norm_val:.4f}")
                        else:
                            parts.append(f"[{stage}: waiting for first forward pass]")
