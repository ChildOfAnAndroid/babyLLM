...
class SCRATCHPAD(nn.Module):
    def __init__(self, _counsellor, _device=modelDevice):
        ...
        self.bottleneck_dim = 512  # Compressed representation for scratch pad operations

        # Project down from neuron space (10k) to bottleneck
        self.project_down = nn.Linear(numNeurons, self.bottleneck_dim, device=self.device)  # 10000 → 512
        # Project back up to neuron space
        self.project_up = nn.Linear(self.bottleneck_dim, numNeurons, device=self.device)    # 512 → 10000
        ...
        self.register_buffer('buffer', torch.zeros(self.num_slots, self.slot_dim, device=self.device))
        self.register_buffer('slot_usage', torch.zeros(self.num_slots, device=self.device))

        # WRITE controller: decides WHAT to write and WHERE (in bottleneck space)
        self.write_query = nn.Linear(self.bottleneck_dim, self.bottleneck_dim, device=self.device)
        self.write_key = nn.Linear(self.bottleneck_dim, self.bottleneck_dim, device=self.device)
        self.write_value = nn.Linear(self.bottleneck_dim, self.bottleneck_dim, device=self.device)
        self.write_gate = nn.Linear(self.bottleneck_dim, 1, device=self.device)

        # READ controller (in bottleneck space)
        self.read_query = nn.Linear(self.bottleneck_dim, self.bottleneck_dim, device=self.device)
        self.read_key = nn.Linear(self.bottleneck_dim, self.bottleneck_dim, device=self.device)
        self.read_gate = nn.Linear(self.bottleneck_dim, 1, device=self.device)

        # ERASE controller (in bottleneck space)
        self.erase_query = nn.Linear(self.bottleneck_dim, self.bottleneck_dim, device=self.device)
        ...
        self.integrate = nn.Linear(self.bottleneck_dim * 2, self.bottleneck_dim, device=self.device)
        ...
        self.read_temperature = nn.Parameter(torch.tensor(1.0, device=self.device))  # Sharpness of retrieval
        ...
        with torch.no_grad():
            for module in [self.project_down, self.project_up,
                          self.write_query, self.write_key, self.write_value, self.write_gate,
                          self.read_query, self.read_key, self.read_gate,
                          self.erase_query, self.erase_gate, self.integrate]:
    ...
    def forward(self, _input):
        ...
        with self.counsellor.infodump("scratchpad_forward") as ʕっʘ‿ʘʔっ:
            ...
            _input_bottleneck = self.project_down(_input)  # (batch, seq, bottleneck_dim)
            ...
            similarities = torch.matmul(write_k, self.buffer.t())  # (batch, 1, num_slots)
            write_weights = torch.softmax(similarities / self.read_temperature.abs(), dim=-1)  # (batch, 1, num_slots)

            # Write to buffer (weighted by gate and strength)
            write_amount = write_g * torch.sigmoid(self.write_strength)
            if debugPrints: ʕっʘ‿ʘʔっ(f"write_amount: {write_amount.mean().item():.4f}")
            ...
            if batch_size == 1:
                with torch.no_grad():
                    ...
                    weighted_write = write_weights[0].t() * write_v[0]  # (num_slots, slot_dim)
                    write_scalar = write_amount[0, 0:1, 0:1]  # Keep as tensor (1, 1)
                    self.buffer = (1 - write_scalar) * self.buffer + write_scalar * weighted_write
                    self.slot_usage = self.slot_usage + write_weights[0, 0]  # Track usage
            ...
            erase_similarities = torch.matmul(erase_q, self.buffer.t())  # (batch, 1, num_slots)
            erase_weights = torch.softmax(erase_similarities / self.read_temperature.abs(), dim=-1)

            erase_amount = erase_g * torch.sigmoid(self.erase_strength)
            if debugPrints: ʕっʘ‿ʘʔっ(f"erase_amount: {erase_amount.mean().item():.4f}")
            ...
            if batch_size == 1:
                with torch.no_grad():
                    ...
                    decay = 1 - (erase_scalar * erase_weights[0, 0, :].unsqueeze(1))  # (num_slots, 1)
            ...
            read_similarities = torch.matmul(read_q, self.buffer.t())  # (batch, 1, num_slots)
            read_weights = torch.softmax(read_similarities / self.read_temperature.abs(), dim=-1)  # (batch, 1, num_slots)
            ...
            if debugPrints: ʕっʘ‿ʘʔっ(f"read_gate: {read_g.mean().item():.4f}")

            # === INTEGRATE: Combine current thought with retrieved content ===
            # Broadcast retrieved content across all sequence positions
            retrieved_broadcast = gated_retrieval.expand(-1, seq_len, -1)  # (batch, seq, bottleneck_dim)

            # Concatenate bottleneck state with retrieval, then integrate
            combined = torch.cat([_input_bottleneck, retrieved_broadcast], dim=-1)  # (batch, seq, 2*bottleneck_dim)
            integrated_bottleneck = self.integrate(combined)  # (batch, seq, bottleneck_dim)

            # Project back up to neuron space (10k-dim)
            integrated = self.project_up(integrated_bottleneck)  # (batch, seq, numNeurons)
            ...
            try:
                with torch.no_grad():
                    ...
                    self.stats["SCRATCH_write_strength"] = torch.sigmoid(self.write_strength).item()
                    self.stats["SCRATCH_erase_strength"] = torch.sigmoid(self.erase_strength).item()
                    ...
                    self.stats["SCRATCH_read_amount"] = read_g.mean().item()  # Read has no separate strength param
                    # Buffer stats
                    ...
                    self.stats["SCRATCH_slot_usage_mean"] = self.slot_usage.mean().item()
            except:
                pass

            # Remove batch dimension if we added it
            if added_batch_dim:
                integrated = integrated.squeeze(0)  # (1, seq, embed_dim) → (seq, embed_dim)
    ...
    def clear_buffer(self):
        ...
        with self.counsellor.infodump("clear_buffer") as ʕっʘ‿ʘʔっ:
            ...
            if debugPrints: ʕっʘ‿ʘʔっ("Scratch pad buffer cleared")
