...
def makeDatBoi():
    ...
    pointLeft = random.choice([True, False])
    if pointLeft:
        datBoi = random.choice(leftArmsOut) + datBoi  # '⊂'
    # throwLeft = random.choice([True, False])
    # if throwLeft: datBoi = " " + datBoi #' ⊂'
    thingyLeft = random.choice([True, False])
    if thingyLeft:
        datBoi = random.choice(thingys) + datBoi  # '❀⊂' / '❀ ⊂'

    # 'ʕ'/'⊂ʕ'/' ⊂ʕ'/'❀⊂ʕ'/'❀ ⊂ʕ'/'❀ʕ'/'❀ ʕ'

    leftHair = random.choice([True, False])
    if leftHair and thingyLeft is False:
        datBoi += random.choice(hairThingys)  # 'ʕ❀'/'⊂ʕ❀'/' ⊂ʕ❀'/'❀⊂ʕ❀'/'❀ ⊂ʕ❀'
    if pointLeft is False and leftHair is False:
        hugLeft = random.choice([True, False])
        if hugLeft:
            datBoi += random.choice(leftArmsIn)  # 'ʕ⊃'/'❀ʕ⊃'/'❀ ʕ⊃'
    ...
    hugRight = random.choice([True, False])
    if hugRight:
        datBoi += random.choice(rightArmsIn)
    rightHair = random.choice([True, False])
    if rightHair and hugRight is False and leftHair is False and thingyLeft is False:
        datBoi += random.choice(hairThingys)
    datBoi += random.choice(rightSides)

    if hugRight is False:
        pointRight = random.choice([True, False])
        if pointRight:
            datBoi += random.choice(rightArmsOut)

    # throwRight = random.choice([True, False])
    # if throwRight: datBoi += " "
    thingyRight = random.choice([True, False])
    if thingyRight and rightHair is False and leftHair is False and thingyLeft is False:
        datBoi += random.choice(thingys)
