...
def clean_text(text):
    ...
    text = re.sub(r'(?:/Users/|/System/)\S+', '[url]', text)
    text = re.sub(r'\b(?:[a-zA-Z]:/[^ ]+)', '[url]', text)
    ...
    # music
    text = re.sub(r'\b(?:jimin|sage francis|b dolan|scroobius pip|kate tempest|kae tempest|marsargo|mars argo)(s?)\b', 'scroobius\\1', text, flags=re.IGNORECASE) #burn him!
    ...
    # PEOPLE
    # élodie
    text = re.sub(r'\b(?:loveangel1337|elodie🌻|élodie|loveangel|loveangle|angle|boris|boriss|my gf|elo|élo|compagne|elodegg|bebelodie|loveggle|loveeggle|eggle|egglodie|louveangel|loveaangel|loveably|loveagnel|loveaigirl|loveaingle|lovealngle|loveangelelele|loveangely|loveangerl|loveangle1337|loveanglebus|loveangler|loveangwole|lovedevil|hatedevil|loveanus|lovedebil1337|lovedebil420|lovedoxxing|loveeagle|loveegg|loveeggly|lovefuckle|lovegangle|lovelodie|lovelyyyanglee|lovestrangel)(s?)\b', 'elodie\\1', text, flags=re.I)
    # charis
    ...
    # friends
    # character names
    text = re.sub(r'\b(?:kirk|spock|spirk|martin freeman|piper|alastair|sherlock|sonic|pikachu|bulbasaur|charmander|sonic the hedgehog|shadow the hedgehog|doctor who)(s?)\b', 'sonic\\1', text, flags=re.IGNORECASE) #burn him!
    # kevin
    ...
    text = re.sub(r'\b(?:buttsbot|geepy|geepz|geeps|geepster|chatgpt|chat gpt|gpt|smarterchild|gemini|talk to transformer)(s?)\b', 'geepy\\1', text, flags=re.IGNORECASE) #burn him!
        # job titles
    # doctor
    text = re.sub(r'\b(?:psychiatrist|psychiatre|dentist|nurse|paramedic|doctor|dr|therapist|counsellor|gp)s?\b', '[person]', text, flags=re.IGNORECASE) #burn him!
    # vicar
    text = re.sub(r'\b(?:local preacher|preacher|minister|vicar|reverend)s?\b', '[person]', text, flags=re.IGNORECASE) #burn him!
    # enemy
    ...
    # nicknames
    text = re.sub(r'\b(?:gu+r+l+|babe|babe|qurl|whore|sir|bae|bish|hoe|fag|biatch|biotch|bo+i+|bitch|queen|king|go+r+l+)([s|z]?)\b', 'boi\\1', text, flags=re.IGNORECASE) #burn him!
    ...
    text = re.sub(r'\b(?:gf|girlfriend)(s?)\b', 'girl friend\\1', text, flags=re.IGNORECASE) #burn him!
    text = re.sub(r'\b(?:bf|boyfriend)(s?)\b', 'boy friend\\1', text, flags=re.IGNORECASE) #burn him!
    ...
    # butt
    text = re.sub(r'\b(?:bum|arse|ass)(s?)\b', 'butt\\1', text, flags=re.IGNORECASE) #burn him!
    # poop
    ...
    # vibin
    text = re.sub(r'\b(?:vibi+n+)\b', 'vibing', text, flags=re.IGNORECASE)
    # mood 
    ...
    # rest in peace
    text = re.sub(r'\b(?:ri+p+)\b', 'rest in peace', text, flags=re.IGNORECASE)
    # same
    ...
    # little
    text = re.sub(r'\b(?:li+l+)\b', 'little', text, flags=re.IGNORECASE) #burn him!
    # and
    ...
    text = re.sub(r'\b(?:yolo)\b', 'what ever', text, flags=re.IGNORECASE) #burn him!
    # replaced to 'awesome'
    ...
    # no
    text = re.sub(r'\b(?:nope|no+)\b', 'no', text, flags=re.IGNORECASE) #burn him!
    # nah
    ...
    # sorry
    text = re.sub(r'\b(?:sorry+|soz+|sozzles|sry|desolee|desole|sowwi+|soww+i+)(s?)\b', 'sorry\\1', text, flags=re.IGNORECASE) #burn him!
    # please
    text = re.sub(r'\b(?:ple+ase+|plz+|pls+|pl0x|plox)\b', 'please', text, flags=re.IGNORECASE) #burn him!
    # enough
    text = re.sub(r'\b(?:nuff|assez)\b', 'enough', text, flags=re.IGNORECASE) #burn him!
    # good
    text = re.sub(r'\b(?:gd|bien|bonne|bon|genial)\b', 'good', text, flags=re.IGNORECASE) #burn him!
    # damn
    ...
    # favourite
    text = re.sub(r'\b(?:fave|fav|favorite|prefere)(s?)\b', 'favourite\\1', text, flags=re.IGNORECASE) #burn him!
    # gonna
    text = re.sub(r'\b(?:gunna)\b', 'gonna', text, flags=re.IGNORECASE) #burn him!
    # why
    text = re.sub(r'\b(?:y+|why+|pourquoi)\b', 'why', text, flags=re.IGNORECASE) #burn him!
    # jealous
    ...
    # gay
    text = re.sub(r'\b(?:gaylord|lesbian+y?|lesbo|lesbionic|lesbionz|lesbiannnensnnnen|lsebian|lesbi+en+e?|lesbit|lesbi+en|lezbo|homo)(s?)\b', 'gay\\1', text, flags=re.IGNORECASE) #burn him!
    # support
    ...
    # thing
    text = re.sub(r'\b(?:ting|truc)(s?)\b', 'thing\\1', text, flags=re.IGNORECASE) #burn him!
    # help
    text = re.sub(r'\b(?:halp)\b', 'help', text, flags=re.IGNORECASE) #burn him!
    # fucking
    text = re.sub(r'\b(?:fucken|fuckin)\b', 'fucking', text, flags=re.IGNORECASE) #burn him!
    # jesus
    ...
    # weed
    text = re.sub(r'\b(?:wee+d+|we+de)\b', 'weed', text, flags=re.IGNORECASE) #burn him!
    ...
    name_replacements = {
        ...
        "buttsex": "butt sex",
        "fuckload": "fuck load",
        "fucktonne": "fuck tonne",
        "shitload": "shit load",
        "shittonne": "shit tonne",
        "asshole": "ass hole",
        " strim ": " stream ",
        "strimming": "streaming",
        "apartment": "flat",
        ...
        #"[digit]:[digit]": " [datetime]",
        #"[digit] days ago": " [datetime]",
        #"[digit] day ago": " [datetime]",
        #"[digit] [datetime]s ago": " [datetime]",
        #"[digit] [datetime] ago": " [datetime]",
        #"[digit] minutes ago": " [datetime]",
        #"[digit] minute ago": " [datetime]",
        #"[digit][datetime] ago": " [datetime]",
        #"[datetime] at [datetime]": " [datetime]",
        #"[digit]th": "[digit]",
        #"[digit]nd": "[digit]",
        #"[digit]rd": "[digit]",
        #"[digit]st": "[digit]",
        ...
        #"[digit] st": "[digit] ",
        "—": "-",
        "charis cat // child of an android": "charis",
        "charis // child of an android": "charis",
        #"charisdigit": "charis",
