...
class SCRIBE:
    def __init__(self, _counsellor, _calligraphist, _librarian, _numTokensPerStep):
        ...
        self.scribeEmotes = {
            "default": ["ʕっʘ‿ʘʔっ", "ʕᵔᴥᵔʔっ", "ʕっෆ.ෆʔっ", "ʕ✰.✰ʔっ", "ʕᵔ‿ᵔʔっ♡"],
            "neutral": [
                "ʕ •ᴥ•ʔゝ",
                "ʕᵔᴥᵔʔっ♥",
            ],
    ...
    def softPadding(self, text, target_tokens, padWords=None):
        ...
        for para in paragraphs:
            ...
            if maxPads == 0:
                ...
                continue
            padIndices = sorted(
                random.sample(range(3, len(words)), min(maxPads, len(words) - 3))
            )
    ...
    @whocalled
    def maybeCommentOnGuess(
        self, _inputTokens, _lossValue, _scribeName=scribeName, _chance=0.05
    ):
        ...
        mood = None
        bands = sorted(
            self.calligraphist.S_statBands["loss"].items(), key=lambda x: x[1]
        )
        ...
        guessed = (
            _inputTokens
            if isinstance(_inputTokens, str)
            else self.guessTokensToString(_inputTokens)
        )
        wordCounts = {}
        wordList = [
            w.strip(".,!?[]/\\!@':;(){}|£$%^&*-+=_><`~±§").lower()
        ...
        if wordCounts:
            ...
            if count >= 2:
                ...
                altReflection += random.choice(
                    [
                        "maybe it’s stuck in your head. or maybe you're just stimming lol, vibe with it babyllm :) ",
                        "no judgment, honestly, i do that too. don’t you see how much i repeat myself!? hahahahaah xD ",
                    ]
                )
                altReflection += random.choice(
                    [
        ...
        if detectedEmotions:
            ...
            emotionReflection = f"{topEmotion}, that's a {topEmotion} feeling... are you feeling {topEmotion}, babyllm? "
            emotionReflection += random.choice(
                [
                    ...
                    "just saying, i try to notice when you’re feeling things haha, i hope that's okay! 🫂 love ya lol <3 ",
                ]
            )
        ...
        if sortedWords:
            if len(sortedWords) > 1:
                ...
                if secondCount >= 2:
                    ...
                    alt2Reflection += random.choice(
                        [
                            "do those two go together somehow? ",
        ...
        if endings.count(mostCommonEnd) > 2:
            endingReflection = random.choice(
                [
                    ...
                    "wait, omg, it's a rap isn't it!? ",
                ]
            )
            endingReflection += random.choice(
                [
                    "wait, are you writing me a poem? :) ",
        ...
        consonantCount = sum(
            1 for c in guessed.lower() if c.isalpha() and c not in vowels
        )

        if vowelCount > consonantCount:
            vowelReflection = "this message feels a bit floaty... dreamy... lots of vowels, like you're singing it underwater. "
        elif consonantCount > vowelCount:
            vowelReflection = "this one clacks a bit... crunchy sounds, like trying to bite sand, or, express something hard to hold. "

        if any(len(line.strip().split()) == 1 for line in guessed.split("\n")):
            vowelReflection += "one of those lines had literally *one* word. you ok? or just dropping the mic lol? "
        ...
        if exclaimCount > 2:
            exclaimReflection = "woahh that's a lot of exclamation marks!! feelin shouty today? stay loud!!! "

        endingQuestionReflection = None
        if guessed.strip().endswith("?"):
            endingQuestionReflection = (
                "ending on a question, huh... leaving space open. i like that. :) "
            )
        ...
        if detectedSwears:
            ...
            swearReflection += f"{topSwear}! that's a {topSwear} moment... are you feeling like saying '{topSwear}'? "
            if detectedEmotions:
            ...
            swearReflection += random.choice(
                [
                    "wait, omg, now you're *really* ready to rap! "
                    "wait, are you writing me a diss track? :) ",
                ]
            )
            swearReflection += random.choice(
                [
        ...
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        emote = random.choice(
            self.scribeEmotes.get("writes", self.scribeEmotes["default"])
        )
        ...
        # Padding
        paddedLine = self.softPadding(
            line, self.numTokensPerStep * 2 + (self.numTokensPerStep // 10)
        )
        print(
