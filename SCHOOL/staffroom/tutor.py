...
class TUTOR:
    ...
    def scribeLesson(self, scribeMessage):
        with self.counsellor.infodump("scribeLesson") as ʕっʘ‿ʘʔっ:
            ...
            self.predictedTokenIndices, _ = self.trainStep(
                inputTokenIndices, targetTokenIndices, None
            )
    ...
    def interactiveLearning(
        ...
        with self.counsellor.infodump("interactiveLearning") as ʕっʘ‿ʘʔっ:
            ...
            if len(inputSeqIDs) != len(targetSeqIDs) or len(inputSeqIDs) < 1:
                print(
                    f"skipping interactiveLearning: input/target mismatch or too short (input={len(inputSeqIDs)}, target={len(targetSeqIDs)})"
                )
                return False

            if debugPrints:
                ʕっʘ‿ʘʔっ("entering interactiveLearning...")
            if debugPrints:
                print(
                    f"DEBUG interactiveLearning: _last_loss_delta = {self.latestLossDelta}"
                )

            # start turn actions
            if debugPrints:
                ʕっʘ‿ʘʔっ("trying to call startTurnActions...")
            try:
                self.inputTokenIndices, self.targetTokenIndexSeq = (
                    self.startTurnActions(
                ...
                if debugPrints:
                    ʕっʘ‿ʘʔっ(f"ERROR INSIDE startTurnActions: {e_start_turn}")
                self.model.optimizer.zero_grad(set_to_none=True)
                empty_mps_cache()
                return False
            ...
            if debugPrints:
                print(
                    f"DEBUG interactiveLearning: BEFORE calling trainStep, self.totalTries = {self.totalTries}"
                )
            if debugPrints:
                print(
                    f"DEBUG interactiveLearning: does tutor object have totalTries? {hasattr(self, 'totalTries')}"
                )
            if debugPrints:
                ʕっʘ‿ʘʔっ("trying to call trainStep...")
            try:
                self.predictedTokenIndices, self.logitSeq = self.trainStep(
            ...
            if not self.predictedTokenIndices:
                print(
                    "DEBUG interactiveLearning: trainStep failed to return predicted_token_indices. Exiting."
                )
                ʕっʘ‿ʘʔっ("trainStep failed in interactiveLearning")  # Counsellor log
                return False
            if debugPrints:
                print(
                    "DEBUG interactiveLearning: trainStep returned predictedTokenIndices."
                )

            # collect turn stats
            if debugPrints:
                ʕっʘ‿ʘʔっ("trying to call collectTurnStats...")
            try:
                self.stats, self.stringStats, self.guessedTokenSeq = (
                    self.collectTurnStats()
                )
                ...
                if debugPrints:
                    print(f"ERROR in collectTurnStats: {e_collect_stats}")
                self.model.optimizer.zero_grad(set_to_none=True)
                empty_mps_cache()
                return False
            ...
            if showStats:
                ...
                dummy_training_pairs_for_log = (
                    [(None, None)] * currentTotalPairs if currentTotalPairs > 0 else []
                )

                if debugPrints:
                    ʕっʘ‿ʘʔっ("calling logFreqActions...")
                self.logFreqActions(
                ...
                if debugPrints:
                    ʕっʘ‿ʘʔっ("calling S_colourPrintTraining...")
            ...
            self.latestLossDelta = self.stepLossFloat - self.averageRecentLoss
            self.totalAvgLoss = (
                (self.totalAvgLoss * (self.totalTurns - 1) + self.stepLossFloat)
                / self.totalTurns
                if self.totalTurns > 0
                else self.stepLossFloat
            )
            absdelta = abs(self.latestLossDelta)
            self.totalLossAbsDelta += absdelta
            self.totalAvgAbsDelta = self.totalLossAbsDelta / max(1, self.totalTurns)
            ...
            # end turn actions
            if debugPrints:
                ʕっʘ‿ʘʔっ("calling endTurnActions...")
            self.latestLossDelta = self.endTurnActions()

            if debugPrints:
                print("--- finished interactiveLearning successfully :) ---")
            return True
